...
function timestep!(Ψ::WindowMPS, H::Window, t::Number, dt::Number,alg::TDVP,env::Window=environments(Ψ,H); leftevolve=true, rightevolve=true)
    ...
    if leftevolve
        # println("In windowtdvp: Psi.left_gs is of type $(typeof(Ψ.left_gs))\n and HLeft is of type $(typeof(H.left))")
        # println("In windowtdvp: nleft is of type $(typeof(nleft))\n and env is of type $(typeof(env))")
        # println("Types of the elements of the window are")
        # println("left: $(typeof(env.left))")
        # println("middle: $(typeof(env.middle))")
        # println("right: $(typeof(env.right))")
        nleft, _ = timestep(Ψ.left_gs, H.left, t, dt, alg, env.left; leftorthflag = true) #env gets updated in place
        _update_leftEnv!(nleft, Window(env.left,env.middle,env.left)) # change left to right, such that they are still of the same type
    ...
    for i in 1:(length(Ψ)-1)
        ...
        # println("the types of the arguments in integrate are:")
        # println(typeof(h_ac))
        # println(typeof(Ψ.AC[i]))
        # println(typeof(t))
        # println(typeof(-1im))
        # println(typeof(dt/2))
    ...
    if rightevolve
        ...
        _update_rightEnv!(nright, Window(env.left,env.middle,env.left))
